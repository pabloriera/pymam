...
def peakdetect_fft(y_axis, x_axis, pad_len = 5):
    ...
    n = lambda x: int(np.log(x)/np.log(2)) + 1
...
def peakdetect_sine(y_axis, x_axis, points = 9, lock_frequency = False):
    ...
    y = A * sin(2 * np.pi * f * x - tau) to the peaks. The amount of points used
    ...
    if lock_frequency:
        func = lambda x, A, tau: A * np.sin(2 * np.pi * Hz * (x - tau) + np.pi / 2)
    else:
        func = lambda x, A, Hz, tau: A * np.sin(2 * np.pi * Hz * (x - tau) + 
                                                np.pi / 2)
